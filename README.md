...
## 🖇️ Связи между объектами 🖇️

| **Routes**: | **RouteStops**: | **ScheduleChanges**: | **Tickets**: |
| --- | --- | --- | --- |
| Связь с **Trains** через `TrainID` (многие к одному). | Связь с **Routes** через `RouteID` (многие к одному). |  Связь с **Routes** через `RouteID` (многие к одному). | Связь с **Routes** через `RouteID` (многие к одному).|
| Связь с **Stations** через `DepartureStationID` и `ArrivalStationID` (многие к одному). | Связь с **Stations** через `StationID` (многие к одному). || Связь с **Passengers** через `PassengerID` (многие к одному). |
...
## 🤯  О чем я подумала когда уже все сделала 🤯
   **Триггеры!!** и **функции!!** Как было бы хорошо не вручную все менять при покупке или продаже билета, а настроить триггер. Eще конечно было бы отлично избавиться от всех лишних слов типа "плацкарт" или "купе", хранить их в отдельной таблице с типами мест и тащить их по ID. Отличной идеей было бы добавить еще одно поле в **Routes** где был бы показан статус поезда, следует ли он по расписанию/задерживается/отменен/идет без остановок и также написать функции и триггеры. 
